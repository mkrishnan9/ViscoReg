...
n_loss_type, normalize_normal_loss, unsigned_n, unsigned_d, loss_type, seed, encoder_type, \
    ...
    args.normalize_normal_loss, args.unsigned_n, args.unsigned_d, args.loss_type, args.seed, \
    args.encoder_type, args.model_dirpath, args.inter_loss_type

# set up backup and logging
log_file, log_writer_train, log_writer_test, model_outdir = utils.setup_logdir(logdir, args)
os.system('cp %s %s' % (__file__, logdir))
os.system('cp %s %s' % ('../models/Net.py', logdir))
os.system('cp %s %s' % ('../models/losses.py', logdir))

# get data loaders
torch.manual_seed(0)
np.random.seed(0)
train_set = basic_shape_dataset1d.Segment1D(n_points, n_samples, args.grid_res,
                                             args.nonmnfld_sample_type,
                                             nonmnfld_range=args.nonmnfld_range)
train_dataloader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True,
                                                num_workers=2, pin_memory=True)
torch.manual_seed(seed)
np.random.seed(seed)

# get model — in_dim=1 for 1D input
device = torch.device("cuda:" + str(gpu_idx) if torch.cuda.is_available() else "cpu")
SINR = Net.Network(latent_size=latent_size, in_dim=1, decoder_hidden_dim=args.decoder_hidden_dim,
...
if args.pretrain_model_path:
    utils.log_string(f"Loading pretrained weights from: {args.pretrain_model_path}", log_file)
    SINR.load_state_dict(torch.load(args.pretrain_model_path, map_location=device))
elif args.refine_epoch != 0:
...
criterion = Loss(weights=list(args.loss_weights),
...
for epoch in range(num_epochs):
    if epoch <= args.refine_epoch and refine_flag and args.refine_epoch != 0:
        scheduler.step()
        continue
    else:
        refine_flag = False
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        if batch_idx % 2000 == 0:
            utils.log_string("saving model to file :{}".format('model_%d.pth' % batch_idx), log_file)
        ...
        if batch_idx % 100 == 0:
            ...
            utils.log_string(
                'Epoch: {} [{:4d}/{} ({:.0f}%)] Unweighted: L_Mnfld: {:.5f}, '
                'L_NonMnfld: {:.5f}, L_Nrml: {:.5f}, L_Eknl: {:.5f}, L_Div: {:.5f}'.format(
                    epoch, batch_idx * batch_size, len(train_set),
                    100. * batch_idx / len(train_dataloader),
        ...
        optimizer.step()
        scheduler.step()
        criterion.update_div_weight(epoch * n_samples + batch_idx,
                                    num_epochs * n_samples, args.div_decay_params)

        if batch_idx == num_batches - 1:
            utils.log_string("saving model to file :{}".format('model_%d.pth' % num_batches), log_file)
            torch.save(SINR.state_dict(), os.path.join(model_outdir, 'model_%d.pth' % num_batches))
