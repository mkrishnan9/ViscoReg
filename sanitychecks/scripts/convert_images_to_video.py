...
parser = argparse.ArgumentParser(description="Convert test images into MP4 videos.")
parser.add_argument("--image-path", type=str, required=True, help="Path to the image output directory")
args = parser.parse_args()

# Use the provided image path
image_path = args.image_path # path to image output directory
...
for i in range(n_files):
    ...
    out_sdf.write(sdf_img); 
    #out_eikonal.write(eikonal_img); out_div.write(div_img); out_zdiff.write(zdiff_img);
...
out_sdf.release();
#out_sdf_eikonal_div.release(); out_eikonal.release(); out_div.release(); out_curl.release(); out_zdiff.release(); out_sdf_eikonal_div_zdiff.release();