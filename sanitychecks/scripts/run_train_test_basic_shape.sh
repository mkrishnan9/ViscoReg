...
mkdir -p $BASE_LOGDIR
FILE=`basename "$0"`
cp scripts/$FILE $BASE_LOGDIR # saves copy of this script so you know the args used
...
#############################
LOSS_TYPE='siren_wo_n_w_visc' # 'siren_wo_n_w_div' | 'siren_wo_n' | 'siren_w_div' | 'siren'
...
for SHAPE in 'blob'
do
  LOGDIR=${BASE_LOGDIR}${NONMNFLD_SAMPLE_TYPE}'_sampling_'${GRID_RES}'/'${SHAPE}'/'${IDENTIFIER}'/'
  ...
  python3 test_basic_shape.py --logdir $LOGDIR --gpu_idx $GPU --epoch_n "${EPOCHS_N_EVAL[@]}" --neuron_type ${NEURON_TYPE}
done
