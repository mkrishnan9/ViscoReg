...
class BasicShape2D(data.Dataset):
    ...
    def __init__(self, n_points, n_samples=128, res=128, sample_type='grid', sapmling_std=0.005,
        ...
        self.nonmnfld_dist = np.ones((self.grid_res**2,)) 
        self.nonmnfld_n = np.ones((self.grid_res**2,2))
        self.dist_img = np.ones((self.grid_res,self.grid_res))
        #breakpoint()
        ...
        # self.grid_dist = np.zeros((self.grid_res**2,))
        # self.nonmnfld_dist = np.zeros((self.grid_res**2,)) 
        # self.nonmnfld_n = np.zeros((self.grid_res**2,2))
        # self.dist_img = np.zeros(self.grid_res,self.grid_res)
    ...
    def __getitem__(self, index):
        ...
        return {'points' : self.points[index, mnfld_idx, :],  'mnfld_n': self.mnfld_n[index, mnfld_idx, :],  \
                'nonmnfld_dist': nonmnfld_dist, 'nonmnfld_n': nonmnfld_n,
...
def resample_boundary(boundary_pts, num_points):
    """Resample boundary to have a specified number of points."""
    distances = np.sqrt(np.sum(np.diff(boundary_pts, axis=0) ** 2, axis=1))
    cumulative_arc_length = np.insert(np.cumsum(distances), 0, 0)

    fx = interp1d(cumulative_arc_length, boundary_pts[:, 0], kind='linear')
    fy = interp1d(cumulative_arc_length, boundary_pts[:, 1], kind='linear')

    new_arc_lens = np.linspace(0, cumulative_arc_length[-1], num_points)
    new_boundary_pts = np.column_stack([fx(new_arc_lens), fy(new_arc_lens)])
    
    return new_boundary_pts
...
def calculate_normals(points):
    n_samples, n_points, _ = points.shape
    normals0 = np.zeros_like(points[0,:,:])
    sample_idx = 0
    
    for i in range(n_points):
        # Get the neighboring points using finite differences
        # Wrap around the indices to handle the boundary cases
        prev_idx = (i - 1) % n_points  # Previous point (wrap around)
        next_idx = (i + 1) % n_points  # Next point (wrap around)

        # Get the neighboring points
        p1 = points[sample_idx, prev_idx]
        p2 = points[sample_idx, next_idx]
            
        # Calculate the tangent vector (difference between the two neighboring points)
        tangent = p2 - p1
            
        # Compute the unit normal vector (perpendicular to the tangent)
        normal = np.array([tangent[1], -tangent[0]])  # 2D outward normal (rotate 90 CW for CCW curve)
        normal = normal / np.linalg.norm(normal)  # Normalize the normal vector

        # Store the normal
        normals0[i] = normal
    
    normals = np.tile(normals0[None, :, :], (n_samples, 1, 1))
    return normals

def normalize_boundary(boundary_pts, target_half_size=0.5):
    """Scale and shift boundary points to fit within [-0.5,0.5] x [-0.5,0.5]."""
    min_x, max_x = np.min(boundary_pts[:, 0]), np.max(boundary_pts[:, 0])
    min_y, max_y = np.min(boundary_pts[:, 1]), np.max(boundary_pts[:, 1])
    ...
    normalized_pts = (boundary_pts - np.array([center_x, center_y])) * scale

    # Scale down to [-0.5, 0.5] instead of [-1,1]
    normalized_pts *= target_half_size

    return normalized_pts

class Blob(BasicShape2D):
    def __init__(self, *args, threshold=15):
        self.threshold = threshold
        BasicShape2D.__init__(self, *args)


    def get_mnfld_points(self):
        x_start, y_start = -2, -1.5  # Starting point
        width, height = 3, 3  # Region size
        density_per_unit = 250  # Resolution
        threshold=15
        ...
        contours = measure.find_contours(X, level=threshold-1)  # Extract boundary at escape time 7

        # Select the largest connected contour
        boundary = max(contours, key=len)  # Choose the longest contour

        # Convert pixel indices to Mandelbrot coordinates
        boundary_pts = np.array([
            [re[int(pt[1])], im[int(pt[0])]] for pt in boundary
            ])
        num_resampled_points = self.n_points
        resampled_boundary = resample_boundary(boundary_pts, num_resampled_points)
        pts = normalize_boundary(resampled_boundary, target_half_size=0.5)

        points = np.tile(pts[None, :, :], (self.n_samples, 1, 1))
        
        return points.astype(np.float32)
    
    def get_mnfld_n(self):
        normals = calculate_normals(self.points)
...
def get2D_dataset(*args, shape_type='circle'):
    ...
    if shape_type == 'circle':
        ...
        raise Warning("Unsupported shape")
